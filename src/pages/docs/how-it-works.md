...
- **Reads** — `navigate`, `get_html`, `wait_for_selector` — run through `chrome.tabs` and a content script. **No CDP. No `navigator.webdriver`.** There is essentially no automation fingerprint on a read.
