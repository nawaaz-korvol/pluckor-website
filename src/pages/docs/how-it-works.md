...
- **Reads** — `navigate`, `get_html`, `wait_for_selector`, `extract`, `extract_links`, `wait_for_response`, `capture_console`, `select_option`, `go_back`, `go_forward`, `reload`, `capture_requests` (metadata) — run through `chrome.tabs`, `chrome.webRequest`, and a content script. **No CDP. No `navigator.webdriver`.** There is essentially no automation fingerprint on a read.
